  ...
  const {modalSelected} = useSelector(store => store.privateFeed);
    useEffect(() => {
      if (modalSelected.order._id) {
        ...
        localStorage.setItem('MODAL_PRIVATE_FEED_ORDER_STATE', JSON.stringify(modalSelected));
      } else {
        window.history.replaceState(null, "", "/profile/orders");
        localStorage.removeItem('MODAL_PRIVATE_FEED_ORDER_STATE');
      }
    }, [modalSelected])
    ...
    return (
      <div className={styles.wrapper}>
        ...
        {modalSelected.order.status === 'done' ?
          ...
          :
          modalSelected.order.status === 'pending' ?
            <p className={clsx(styles.statusDone, 'text text_type_main-small pb-15')}>Готовится</p>
            :
